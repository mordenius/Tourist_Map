...
var ative_point_of_list;//-- актиная точка в списке
 ...
 $(".Left_Info_Block_2_Frame > div").on("touchmove", function(e) {   
     event.preventDefault();
     event.stopPropagation();
 ...
 $(".buttons > div, .point").on("touchend", function(e) {     
     //event.preventDefault();
     //event.stopPropagation();
	...
	$(".point").click(function() {
     m = $(".activet").children("p").text();
	 m = parseInt(m);
	 //--m = m *70;
	 //--m = m - (m*2);
	 console.log(m, "this_category");
	 if(this_category == "tourist_oject"){
	    scroller($(".scroller"), 0); 
		$(choised_element).css({"transform" : "translate3d(0px, " + l + "px, 0px)"}); 
	 }else if(this_category == "infrastructure") {
		scroller($(".scroller"), 0);
		$(choised_element).css({"transform" : "translate3d(0px, " + l + "px, 0px)"});  
	 }
	});
